...
gulp.task("copy_resources_img", "copys web app img to app dir", function() {
    return vfs.src([
        config.paths.webRoot + "/img/**/*"
    ]).pipe(vfs.dest(config.paths.app + "/img"));
});
gulp.task("copy_resources_fonts", "copys web app fonts to app dir", function() {
    return vfs.src([
        config.paths.webRoot + "/fonts/**/*"
...
if (config.env !== "production") {
	buildTasks = ["copy_resources", "js", "css", "ejs", "locale"];
} else {
	buildTasks = ["copy_resources", "copy", "js", "css", "ejs", "locale"];
}
...
gulp.task("resource_reload", ["copy_resources"], reload);
